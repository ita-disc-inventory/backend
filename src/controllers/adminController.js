...
const adminController = {
  async approveOrder(req, res) {
    try {
      ...
      // Get current order status, cost, and budget
      const { data: orderCostBudget, error: getCostError } =
        await supabaseWithToken
          .from("orders")
          .select("total_cost, program_id, status, programs(program_budget)")
      ...
      // If already arrived, just update status to approved (no budget change)
      if (orderCostBudget.status === "arrived") {
        const { error: orderError } = await supabaseWithToken
          .from("orders")
        ...
        if (orderError) {
          console.error("Error updating status to approved:", orderError);
          return res.status(400).json({ error: orderError.message });
        }
      } else {
        // Normal approval flow: check budget, update status, update budget
        if (
